...
class execute_cmd(object):
	...
	def info(self,argv=''):
		...
		printf(f'数据字典：{LYELLOW}{gVar.data}{RESET}')
		printf('============================')
	...
	def request(self,argv=''):
		if re.search(r'(.+)',argv):
			...
			if status_ok(result):
				printf(f'请求发送成功，返回为{LYELLOW}{result}{RESET}')
			else:
				warnf(f'发送请求出错，返回为{LYELLOW}{result}{RESET}')
		else:
			printf(f'请使用 {LCYAN}request 请求API{RESET} 向CQHttp发送请求(参考https://docs.go-cqhttp.org/api/)')
